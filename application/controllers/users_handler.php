<?php
class Users_handler extends CI_Controller {
...
public function fg_pw(){
	...
	if ($this->form_validation->run() == FALSE)
		...
		$subject = "nevalaszolj";
		$this->mailer($link, $email, $subject);
		$this->load->view('templates/header');
...
public function pw_reset($key){



$data['where']['pwres'] = $key;
	...
	if (!$result->num_rows()){
		...
		else
		{
			...
			$updata['where']['nickname'] = $result[0]['nickname']; 
			$updata['tablename'] = 'user';

			$updata['fset'] = array(
			'password' => 'sha1(concat('.$this->db->escape($this->input->post('password')).', salt))'
			);
...
public function regist(){
	...
	if ($this->form_validation->run() == FALSE)
		...
		$subject = "nevalaszolj";
		$this->mailer($link, $usermail, $subject);
...
private function captcha_maker(){
...
private function mailer($content, $recipient, $subject){
...
public function verify($name, $key){
	...
	if (!$result->num_rows()){
		...
		$updata['where']['nickname'] = $data['where']['nickname'];
...
// If the given email and nickname is already in the db, return false
	if ( strrpos ($str, '@')){
		$array['where']['email'] = $str;
	}
	else{
		$array['where']['nickname'] = $str;
	}
...
// check the given password and nickname
	$data['where']['nickname'] = $this->input->post('username');
	$email = $this->input->post('email');
	if ( $email != false){
		$data['where']['email'] = $str;
	}
	else{
		$data['fwhere']['password'] = 'sha1(concat('.$this->db->escape($str).', salt))';
	}
	$result = $this->users_model->arg_check('user', $data);
	if (!$result->num_rows()){
